...
截至 2026-05-22 T135 回归记录时，T131-T134 已单独提交，后端全量 `mvn test -DskipITs` 通过 459 tests；后续接手仍必须每轮先检查 `git status` / `git diff`，避免覆盖用户本地改动。
...
- T123 已完成后端全量回归：`cd backend; mvn test -DskipITs` 通过，452 tests。T116-T122 后仍未新增公开 API、DB 表、Maven 依赖或 docker-compose；未读取 `backend/.env`；未真实调用 provider；未真实连接 Qdrant；未处理 T010/MQ blocked。
...
- T111 已增强 RAG QA trace 脱敏测试和 fallback reason 防护：`RagQaTraceFormatterTest` 覆盖 fallbackUsed=true、contextTruncated=true、retrievedCount=0、citationCount>0、cacheKeyRagAware=true、contextHashPresent=true，以及不输出正文、prompt、provider response、Authorization 等字段。`RagQaTrace.safeFallbackReason` 对明显不安全的 fallback reason 统一输出 `redacted_fallback_reason`，正常 `qdrant_http_error` 等安全 reason 保持原样。
- T110 已补 RAG retrieval eval 失败 / 边界样例：`rag-retrieval-eval-cases.json` 新增 same-keyword-wrong-topic、topK > available chunks 和 metadata isolation negative；`RagRetrievalEvaluationTest` safe report 与 `offline-retrieval-evaluation.json` / `.md` artifact 新增 case-level summary，明确 expectedHit、expectedMarker label、retrievedCount、hit、miss、passed。artifact 仍只使用 synthetic fixture 和本地 fake Qdrant server，不保存文档正文、prompt、endpoint 原文、Authorization、API key、baseUrl 或 provider response。
- T109 已增强离线 RAG vector store demo summary：`RagVectorStoreOfflineDemoTest` 生成的 `target/rag-demo/rag-vector-store-offline-demo-summary.json` 新增 `retrievalSummaries`，覆盖 in-memory smoke、本地 fake Qdrant smoke 和 Qdrant fallback smoke。每条记录只输出 vectorStoreType、fake embedding、sampleId / documentId、短 query label、topK、retrievedCount、score summary、citation metadata presence summary、fallbackUsed / fallbackReason 和 contextHashPresent；不输出文档正文、prompt、endpoint 原文、Authorization、API key、baseUrl 或 provider response。
...
- T102 已增强内部 RAG QA trace formatter：`RagQaTraceFormatter.toInterviewSafeMap` / `formatInterviewSummary` 固定输出 ragEnabled、embeddingProvider、vectorStoreType、topK、retrievedCount、contextHashPresent、contextTruncated、fallbackUsed、fallbackReason、citationCount、indexReused、cacheKeyRagAware，适合 demo / 面试展示；未新增公开 API，未输出文档正文、prompt、endpoint、Authorization、API key、baseUrl 或 provider response。后端 full test 已更新通过到 437 tests。
- T101 已新增离线 RAG eval artifact 生成器：`backend/scripts/rag/run-rag-evaluation-artifact.ps1` 触发 `RagRetrievalEvaluationArtifactTest`，生成 `docs/ai-dev/benchmarks/rag/offline-retrieval-evaluation.json` 与 `.md`。artifact 使用 synthetic fixture，覆盖 in-memory positive hit rate、no-match、empty document、多 documentId isolation、本地 fake Qdrant retrieval 和 Qdrant fallback row；只保存聚合摘要，不保存文档正文、prompt、endpoint 原文、Authorization、API key、baseUrl 或 provider response。后端 full test 已更新通过到 436 tests。
...
- RAG retrieval eval runner / report 已稳定化：`RagRetrievalEvaluationTest` 会生成 `target/rag-eval/rag-retrieval-eval-summary.json`，`backend/scripts/rag/run-rag-retrieval-eval.ps1` 可在本地运行 eval 并打印脱敏聚合指标；report 覆盖 hit / miss / hitRate、index reuse 和 document isolation，不输出正文、prompt、endpoint、Authorization 或 provider response。
- RAG retrieval hardening / eval 文档已完成 T093 收口：README、RAG minimal design、vector store selection、adapter boundary、项目面试 brief 和简历 bullet 已同步 T088-T092 事实边界；默认 provider 仍为 `in_memory`，Qdrant 仍默认关闭，当前不是生产完整 RAG。
- RAG retrieval offline eval 已补齐：新增固定小样例 eval 资源和 `RagRetrievalEvaluationTest`，默认用 fake embedding + in-memory vector store 统计 total、hitCount、missCount、hitRate、averageRetrievedCount，并用本地 fake Qdrant server 覆盖一条 adapter eval；当前不调用真实 provider，不启动真实 Qdrant。
- Qdrant collection preflight boundary 已补齐：新增 collection info / create payload builder、response parser 和 preflight result；preflight 脚本默认只读，只有显式 `-AllowCreateCollection` 且 collection 缺失时才会尝试 create。当前未启动真实 Qdrant，未改 docker-compose。
...
- Qdrant 故障 fallback 已补测试和轻量加固：QA RAG context 构建遇到 Qdrant HTTP error / timeout / disabled 时会记录脱敏 reason 并回退普通 QA；Agent `rag_tool` 遇到向量库失败会返回空召回和 `fallbackUsed=true`，不直接抛出 provider 异常。
...
- Qdrant HTTP vector store adapter 已实现但默认关闭：只有显式 `app.rag.vector-store.provider=qdrant` 时才创建 `QdrantVectorStore`；默认仍是 `in_memory`。单元测试使用 JDK 本地 fake HTTP server，不依赖真实 Qdrant。
...
- T068 已重新做真实 embedding provider preflight：`APP_RAG_EMBEDDING_PROVIDER=False`、`APP_RAG_EMBEDDING_BASE_URL=False`、`APP_RAG_EMBEDDING_MODEL=False`、`APP_RAG_EMBEDDING_API_KEY=False`，真实 embedding runtime 仍为 BLOCKED；未发起 `/embeddings` HTTP 调用，未读取 `backend/.env`，未输出 secret、baseUrl、Authorization、request body、provider response 或文档正文。该 BLOCKED 不阻塞后续 fake embedding + in-memory RAG QA smoke。
- T069 已新增 RAG QA fake smoke：`RagQaSmokeVerificationTest` 使用 fake embedding + in-memory vector store + `app.rag.qa.enabled=true` 走 `DocumentQaServiceImpl`，验证 RAG context 注入、retrievedCount、contextHash、citation metadata 和 RAG-aware cache key；同时验证 flag=false 时不调用 RAG builder，普通 QA context 不变。验证：`*Rag*` 21 tests、`DocumentQaServiceImplTest` 36 tests、后端 full test 350 tests 通过。未新增 API、前端、DB、依赖或 docker-compose，未输出文档正文、prompt、provider response 或 secret。
- T070 已新增内部 RAG QA debug trace：`RagQaContext` 携带 `RagQaTrace`，`RagQaTraceFormatter` 只输出脱敏白名单字段；trace 覆盖 ragEnabled、embeddingProvider、vectorStoreType、documentIdPresent、topK、retrievedCount、maxContextChars、contextChars、contextTruncated、contextHashPresent、fallbackUsed、fallbackReason、citationCount、cacheKeyRagAware。验证：`*Rag*` 24 tests、`DocumentQaServiceImplTest` 36 tests、后端 full test 353 tests 通过。未新增 API、Actuator、前端、DB、依赖或 docker-compose，未输出正文 / prompt / secret。
...
- T074 已新增 in-memory RAG index lifecycle：`RagIndexKey` / `RagIndexState` / `RagIndexManager` 按 documentId、documentVersion、contentHash、embedding provider、vector store type 判断复用，`InMemoryVectorStore` 支持按 documentId 替换旧 chunks，QA RAG trace 与 Agent RAG step 摘要可展示 `indexReused=true/false`。同一 document/version/contentHash 会跳过重复 index，version 或 contentHash 变化会重建，不同 documentId 隔离，manager 支持 clear / getState 供内部测试。验证：`*Rag*` 32 tests、`*Agent*` 53 tests、后端 full test 361 tests 通过。
- T075 已新增 Qdrant vector store disabled skeleton：`app.rag.vector-store.provider` 默认 `in_memory`，可选 `qdrant_disabled`；`RagVectorStoreProperties` / `VectorStoreFactory` 负责配置选择，`DisabledQdrantVectorStore` 仅返回本地 disabled 行为，不发 HTTP、不依赖 endpoint / api key。验证：`*Rag*` 37 tests、`VectorStoreFactoryTest` 3 tests、后端 full test 369 tests 通过。未真实接 Qdrant / Redis Vector，未新增依赖、API、数据库表或 docker-compose。
- T076 已完成 RAG demo 与 vector skeleton 文档收口：README、项目面试 brief、简历 bullet、RAG minimal design、vector store selection、adapter boundary 均已同步 T072-T075 事实。当前可讲 fake embedding + in-memory vector store 的 RAG demo、QA RAG feature flag、脱敏 RAG trace、in-memory index lifecycle、Qdrant disabled skeleton；不能写真实 embedding / 真实 Qdrant / 生产完整 RAG 已完成。文档乱码扫描通过，后端 full test 369 tests 通过；未改前端。
