...
1. 继续 `T012d`：新增 selector shadow metrics 的内存态 collector 和 snapshot。
...
原因：T012c 已接入 shadow compare 旁路逻辑，下一步应补齐 selector compare metrics；默认仍必须保持 keyword selector，不真实调用 LLM，也不改变 `/api/ai/agent/run` 行为。
