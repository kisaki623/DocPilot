...
- 2026-07-11 max-stress audit refreshed the small real-provider evidence: Memory provider marker `docpilot-memory-provider-20260711172435-14083e` passed with the fixed 6-call suite, while rerank effect marker `docpilot-rerank-effect-rerank-20260711171449-522a4c` is REVIEW because the configured rerank model returned `NotFound` and the app fell back to `identity`. Until a configured rerank provider run reports `rerankApplied=true`, do not claim current real rerank uplift.
