...
- 2026-07-10 M1 case diversity v2 is DONE: the real-provider contract is now a fixed 6-case sample, adding Chinese durable `PREFERENCE + PROJECT_STATE` extraction and one-time instruction suppression. Final marker `docpilot-memory-provider-20260710204432-4540df` passed with 6 calls, `casePassRate=1.0000`, and `rawProviderOutputStored=false`; malformed JSON / suggestion schema now fail closed, and the wrapper does not persist raw Maven output. Runtime extraction remains rule-based. This remains a small output-semantics contract, not a JSON compliance benchmark or proof of mature long-term memory quality.
- 2026-07-12 rerank representative eval is DONE as bounded multi-case real-link evidence: wrapper `rerank-representative-eval-smoke.ps1` compares hybrid-only baseline with Aliyun Bailian `qwen3-rerank` candidate over 12 representative cases. Final baseline marker `docpilot-rerank-representative-representative-hybrid-20260712151858-5543fd` and candidate marker `docpilot-rerank-representative-representative-rerank-20260712152212-2e0f81` passed with 10/10 target cases covered, 2/2 no-evidence cases preserved, `rerankApplied=true`, `targetRerankAppliedCaseCount=10`, `strictImprovementCaseCount=2`, `upliftCaseCount=10`, `citationLeakageCount=0`, and `noEvidenceRegressionCount=0`. This supersedes the previous “representative eval still pending” item, but remains a small redacted smoke/eval gate rather than a large-scale ranking benchmark.
- 2026-07-12 rerank uplift hard fixture is DONE as small-sample evidence: Aliyun Bailian marker `docpilot-rerank-effect-rerank-20260712015353-cc21a9` reported `rerankApplied=true`, `rerankModel=qwen3-rerank`, no failure reason, and hard fixture rank improved from baseline target rank 2 / distractor rank 1 to rerank target rank 1 / distractor rank 3. This is a bounded hard-fixture uplift proof, not a large-scale ranking benchmark.
- 2026-07-12 earlier rerank provider availability run: Aliyun Bailian marker `docpilot-rerank-effect-rerank-20260712003244-46b2e3` reported `rerankApplied=true`, `rerankModel=qwen3-rerank` and no failure reason, with no core RAG / no-evidence / permission regression. That earlier run remained REVIEW for relevance uplift because the old hard fixture did not observe target / distractor ordering improvement; the later dedicated hard fixture above supersedes only the small-sample uplift status, not the need for broader eval evidence.
...
- 2026-07-12 已补 Memory Governance 边界收口：离线测试覆盖相似 ACTIVE memory、跨用户 resolve 隔离、敏感手动创建 / merge / 系统抽取拦截、一次性指令抑制、assistant RAG evidence 不沉淀；真实 `memory-quality-smoke.ps1 -Mode run` PASS，marker `docpilot-memory-quality-20260712155609-7ba60d`，验证冲突 keep/replace/merge、敏感 edit 拦截和 trace 中 userMemory / ragEvidence 分离。
