...
- 2026-07-12 rerank provider availability refreshed: Aliyun Bailian marker `docpilot-rerank-effect-rerank-20260712003244-46b2e3` reported `rerankApplied=true`, `rerankModel=qwen3-rerank` and no failure reason, with no core RAG / no-evidence / permission regression. The run remains REVIEW for relevance uplift because the hard fixture did not observe target / distractor ordering improvement; do not claim broad real rerank uplift without stronger eval evidence.
