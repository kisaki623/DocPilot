...
3. Artifact 聚合只能读取 ignored 的脱敏结果摘要，默认 root 为 `backend/target/audit`、`backend/target/rag-natural-corpus`、`backend/target/rag-real-qa`、`backend/target/memory-quality`、`backend/target/memory-provider`、`backend/target/agent-quality-eval` 和 `tmp-e2e/docpilot-cloud-quality-smoke`。前端不能直接读取 artifact 原文。
