...
6. 2026-07-12 已验证成功的正式调用方式是 PowerShell here-string + `--prompt $prompt`。长 prompt 先在 Codex 侧脱敏和压缩，再放入变量，不要把 `.env`、云地址、连接串、token、raw prompt、answer 或 evidence context 放进去：

```powershell
$prompt = @'
请基于以下脱敏上下文提出前端体验建议……
'@
gemini.cmd -m gemini-3.5-flash --prompt $prompt
```

7. 避免把正式请求写成裸 `-p`、`-p ""` 或 `stdin | gemini.cmd ... -p " "`。本机 2026-07-12 验证中，裸 `-p` / `-p ""` 会被 CLI 判定为缺少参数，`stdin + -p " "` 曾触发 `404 NOT_FOUND` / `GenerativeService.StreamGenerateContent` 代理错误；后续不要把这条路径作为默认调用方式。若命令行长度真的成为问题，先分批压缩上下文或只传相关片段，再用 `--prompt $prompt`。
8. 当 Gemini CLI 明确不可用、连接失败或超时时，可启动本机 AIStudioToAPI 作为一次受控恢复尝试：
...
10. `--approval-mode auto_edit` 只允许在明确文件范围内尝试；如果出现 503、`INVALID_ARGUMENT`、malformed tool call、空响应或工具流异常，立即降级为“Gemini 提建议，Codex 集成”。
11. 安全边界：
...
12. 验证归属固定为 Codex：最终 lint、build、Playwright、乱码扫描、进程清理和 ai-dev 文档回写都由 Codex 执行；Gemini 输出不能直接写成已验证结果。

## 6.1 子 Agent 调度与安全边界

1. `AGENTS.md` 的“多模型子 Agent 自动调度规则”是角色选择、委派时机、等待条件、并发和成本控制的唯一事实源；`.codex/config.toml` 与 `.codex/agents/*.toml` 只承载实际模型、推理强度、sandbox 和线程配置，不得与该规则冲突。
2. 主 Agent 派发前必须写明目标、文件或系统范围、预期输出和是否允许写入；只读 Agent 可并行，但同一时刻只能有一个 `workspace-write` Agent，且不得与主线程或其他写 Agent 编辑同一文件。
3. 子 Agent 不得因模型、角色或 sandbox 配置而放宽本项目安全边界：不得接触或输出 `.env`、凭据、云地址和连接串；不得执行未经授权的远程破坏性操作、数据库迁移、`git commit` 或 `git push`；所有结果仍由主 Agent 审核、汇总和回写。
