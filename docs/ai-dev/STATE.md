...
1. Phase 2 下一片优先修复真实体验审计暴露的问题：移动端 `/conversations` 横向溢出和 KnowledgeBase 多文档召回覆盖。
