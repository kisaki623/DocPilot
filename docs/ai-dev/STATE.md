...
- RAG Real QA Eval v1 已新增真实链路 smoke 入口 `scripts/smoke/rag-real-qa-eval-smoke.ps1`，支持 `plan` / `dry-run` / `run`，默认 marker 前缀为 `docpilot-rag-real-qa`，artifact 根目录为 ignored 的 `backend/target/rag-real-qa`。当前 wrapper 复用 `cloud-quality-smoke.ps1` 的完整业务质量门禁并输出 real-QA case 类型清单；2026-06-28 已验证 plan / dry-run PASS 和脚本安全测试 2/2 PASS，尚未执行会创建业务数据的 `run`。
...
- RAG Real QA Eval v1 也是离线合成基线：它把 case 组织得更接近真实问答类型和质量指标，但当前仍不代表真实 embedding / rerank / answer provider 的用户体验结论；真实效果需要后续 `run` 模式 smoke 和浏览器 / API runtime evidence 收口。
...
1. 继续 DocPilot Quality Loop v2：先把 RAG Real QA Eval 从离线基线推进到真实链路 smoke runner，再进入 Memory Quality Eval 和 Frontend UX Audit。
