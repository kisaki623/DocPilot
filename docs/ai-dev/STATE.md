...
- Memory Quality Eval v1 已有离线 test-side 门禁和一次真实链路 smoke，但仍不等于长期记忆真实模型抽取或大规模个性化效果评测；当前验证的是规则式候选、状态分层、ACTIVE memory 进入上下文、RAG evidence 不混成 memory、trace 计数和权限边界。
...
1. Quality Loop v2 已完成一轮 RAG Real QA Eval、Memory Quality Eval 和 Frontend UX Audit 闭环，并补过 `360px` / `320px` 极窄移动端、长 memory 检查、KnowledgeBase 技术字段产品化降噪、更难 rerank uplift 离线 fixture 和真实 rerank hard smoke。
2. 下一阶段优先进入 Memory 产品化：把来源解释、候选管理、重复 / 冲突记忆、Trace 可读性和真实会话体验继续从 gate 级推进到产品可用级。
