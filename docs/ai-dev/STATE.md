...
- Agent 工具链已新增内部 `ToolSpec` / `ToolSpecRegistry` 元数据底座，供后续 Function Calling adapter 或统一 executor 演进；现有 typed 工具执行链仍保持兼容。
...
- ToolSpec 目前是内部元数据底座，不等于已接 OpenAI Function Calling、MCP 或通用 ToolCall API。
- Agent 不是多智能体自主规划。
