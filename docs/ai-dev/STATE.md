...
- RAG Quality Upgrade v2 已新增真实链路质量门禁入口 `scripts/smoke/rag-real-quality-smoke.ps1`，可复用云端 MySQL / Qdrant tunnel 与本地 backend/frontend 跑真实 embedding + Qdrant 两文档 RAG 门禁。2026-06-27 的 run 结果为 `REVIEW`：核心链路、chunk、MySQL / Qdrant 一致性、单文档 RAG、KnowledgeBase 两文档 RAG、Conversation Trace、权限隔离和 artifact 脱敏均通过；无关 populated-KB query 仍返回最近证据，需要继续治理 no-evidence 阈值。
...
- RAG Quality Upgrade v2 已能在真实 embedding + Qdrant 链路上暴露 no-evidence 问题，但当前记录是 `REVIEW` 而不是 `PASS`；不能写成 populated KnowledgeBase 无关问题已经可靠拒答。下一阶段必须把 no-evidence / threshold / grounded refusal 做成主链路能力，并用 `rag-real-quality-smoke.ps1 -Mode run` 证明该门禁通过。
...
1. 修复 populated KnowledgeBase 无关问题仍返回 nearest evidence 的问题，把 `noEvidenceThreshold` 从 `REVIEW` 推到 `PASS`。
2. 强化 grounded citation：回答必须基于通过阈值的 evidence，低置信 evidence 不允许伪装成可靠引用。
3. 继续把 Conversation Memory 与 KnowledgeBase evidence 分清：短期上下文、长期记忆、RAG evidence 和 trace 各自可解释。
4. 保持 README / docs 展示口径与真实 smoke 证据一致，不把尚未验证的能力写成已完成生产能力。
