...
- `app.agent.selector.mode=keyword|shadow_llm|llm_execute`。
...
- T062 已完成真实 provider execute runtime 与 fallback 验证：先拆分 LLM selector connect timeout / request timeout，再用当前 shell / 系统环境变量完成 health smoke，并独立验证 summary / QA / RAG 三条 `llm_execute` 路径。三条路径均由真实 provider 返回 JSON decision / toolNames，服务端通过 `ToolRegistry` allowlist 与 required tool 校验后执行对应工具，`fallbackUsed=false`。fallback 已覆盖 provider disabled、provider timeout、非法 toolName、decision / toolNames 不匹配、parser failure / provider exception 等回退 keyword selector 场景。该验证没有读取 `backend/.env`，没有输出 API Key、baseUrl、Authorization、prompt、provider 响应或文档正文。
