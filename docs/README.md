...
2. `docs/ai-dev/CURRENT_TASK.md`：确认当前要做的任务。现在优先推进 `RAG Quality Upgrade v5: chunk structure quality`。
...
当前截图主要服务已有 showcase。RAG no-evidence 和 answer audit 已有 smoke / eval 证据；等 v5 chunk structure quality 和 Conversation Memory 质量门禁继续增强后，再补新的 RAG / Memory 截图会更有价值。
