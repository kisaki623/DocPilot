...
2. `docs/ai-dev/CURRENT_TASK.md`：确认当前要做的任务。现在优先推进 `RAG Quality Upgrade v6: hybrid / rerank production gate`。
...
当前截图主要服务已有 showcase。RAG no-evidence、answer audit 和 chunk structure quality 已有 smoke / eval 证据；等 v6 hybrid / rerank gate 和 Conversation Memory 质量门禁继续增强后，再补新的 RAG / Memory 截图会更有价值。
