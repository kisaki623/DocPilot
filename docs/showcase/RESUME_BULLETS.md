...
项目描述：基于 Spring Boot + Next.js 的文档上传、知识库 RAG、会话记忆和内部质量门禁项目。
...
- 实现单文档 / 多文档 KnowledgeBase RAG，支持 chunk 持久化、Qdrant 检索、引用证据、no-evidence 和权限隔离。
- 设计 Conversation Trace 与用户记忆候选治理，区分短期上下文、ACTIVE memory 和 KnowledgeBase evidence。
- 补充后端单元测试、RAG / Memory / MinIO / RocketMQ / Quality Console smoke 和前端 lint/build 验证，沉淀脱敏展示证据并明确项目能力边界。
...
项目描述：围绕企业文档知识库场景，探索 AI 应用的检索增强、会话记忆、工具路由、质量评测和可观测性治理。
...
- 建设 Agent Quality Console 内部质量控制台，聚合真实 audit / eval artifact，展示 Gate 状态、Eval Catalog、Failure Triage、Trace 定位、Run Comparison 和 token / cost 数值，用于定位 AI 质量回归。
- 实现 Conversation Memory 治理链路，支持 ACTIVE / SUGGESTED / IGNORED 分层、重复 / 冲突提示、候选接受 / 忽略 / 合并，并在 Context Trace 中区分 user memory 与 RAG evidence。
...
项目名称：DocPilot 企业知识库 RAG + Memory 质量平台

项目描述：面向企业文档知识库场景的 Java + Next.js 项目，已实现单文档 / 多文档 RAG、会话记忆、Context Trace、最小 Agent 工具链和内部质量控制台。

- 实现真实链路 RAG 质量审计入口，覆盖 tunnel、backend health、frontend routes、上传 / parse / indexing、chunk quality、MySQL / Qdrant 一致性、单文档 RAG、KnowledgeBase RAG、Conversation Trace、权限隔离和 artifact 脱敏。
- 实现 Agent Quality Console 页面，支持从最新真实 run 查看 Gate、Eval Case、Failure Bucket、Trace Reference、Run Comparison 和 Model / Cost Summary，支撑“发现问题 -> 定位 -> 修复 -> 回归”的展示闭环。
...
- 通过 Maven 测试、前端 lint/build、真实用户 QA 审计、Agent Quality Eval、KnowledgeBase RAG、MinIO、RocketMQ 和真实 embedding smoke 记录沉淀验证证据，明确 Function Calling 未默认接管、Quality Console 不是企业级 APM、当前结果不是线上 SLA 的边界。
...
- 可以讲“已完成单文档 / 多文档 RAG、EmbeddingProvider 抽象、Qdrant adapter、真实 embedding + Qdrant smoke、scope isolation、citations、Conversation Trace、Memory governance、offline eval、真实链路 audit、no-evidence / hard-negative / answer-grounding / answer faithfulness 质量门禁，以及 Agent Quality Console 内部质量控制台”，但不要写“生产级完整向量 RAG / 商业 APM / 大规模 relevance benchmark / 通用语义蕴含模型已上线”。
